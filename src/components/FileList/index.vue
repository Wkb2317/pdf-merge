<script setup lang="ts">
  ...
  import { NUpload, NIcon, NUploadTrigger } from "naive-ui";
  ...
  const uploadChange = async function (options: any) {
    ...
    // re
    const isInList = oldList.some(
      (item: any) => item.file.path === file.file.path
    );
    if (isInList) {
  ...
  const removeItem = function (fileData: UploadFileInfo) {
    fileListRt.value = fileListRt.value.filter(
